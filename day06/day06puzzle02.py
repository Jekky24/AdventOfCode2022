...
while i < len(lines):
    # Convert 14 characters at a time into a set and then check their length. If
    # there are repeating characters, then the length will reduce and no longer
    # be equal to the original length (14). When the first occurence of 4 unique
    # characters in a row is found, the if statement will be true
    if len(set(lines[i:i+14])) == len(lines[i:i+14]):
        # Update the start packet position to be 14 positions after i. Python
        # indexes at 0, but the packet position is indexed at 1, so need to +14
        # instead of +13
        start_packet_pos = i + 14
